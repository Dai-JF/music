    ...
    <el-row :gutter="20" class="mgb20">
      ...
      <el-col :span="4">
        <el-card shadow="hover" :body-style="{ padding: '0px' }">
          <div class="grid-content grid-con-4">
            <div class="grid-cont-right">
              <div class="grid-num">{{ collectionCount }}</div>
              <div>收藏数量</div>
            </div>
          </div>
        </el-card>
...
function setUserSex(sex, arr) {
  let value = 0;
  const name = sex === 0 ? "女" : sex == 1 ? "男" : "未知";
  for (let item of arr) {
    if (sex === item.sex) {
      value++;
    }
...
function setSingerSex(sex, arr) {
  let value = 0;
  const name = sex === 0 ? "女" : sex == 1 ? "男" : sex == 2 ? "未知" : "组合";
  for (let item of arr) {
    if (sex === item.sex) {
      value++;
    }
...
HttpManager.getAllUser().then((res) => {
  ...
  userSex.series[0].data.push(setUserSex(0, res));
  userSex.series[0].data.push(setUserSex(1, res));
  userSex.series[0].data.push(setUserSex(2, res));
...
HttpManager.getAllCollection().then((res) => {
  collectionCount.value = res.length;
});
...
HttpManager.getAllSinger().then((res) => {
  ...
  singerSex.series[0].data.push(setSingerSex(0, res));
  singerSex.series[0].data.push(setSingerSex(1, res));
  singerSex.series[0].data.push(setSingerSex(2, res));
  singerSex.series[0].data.push(setSingerSex(3, res));
